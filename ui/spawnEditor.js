...
import { screenToWorld, getScene, setZoom, resetZoom, getCurrentFrustum, rebuildArenaVisuals } from '../engine/renderer.js';
import { getInputState, consumeInput } from '../engine/input.js';
import { startWave, resetWaveRunner } from '../engine/waveRunner.js';
import { clearEnemies } from '../entities/enemy.js';
import { releaseAllProjectiles } from '../entities/projectile.js';
import { ARENA_HALF, OBSTACLES, PITS } from '../config/arena.js';
import { invalidateCollisionBounds } from '../engine/physics.js';
...
// Level editor state
const levelState = {
  selectedType: null,   // 'obstacle' or 'pit'
  selectedIdx: -1,      // index into OBSTACLES or PITS
};

// Arena preset state
let currentPresetName = null; // name of loaded preset, or null
...
let markers = []; // { mesh, waveIdx, groupIdx, spawnIdx }

// 3D markers for level tab
let levelMarkers = []; // { mesh, type: 'obstacle'|'pit', idx }
...
// DOM references
let waveLabel, groupLabel, spawnCountLabel;
let bannerEl;
let spawnTabContent, levelTabContent;
...
let lastBuiltTuningType = null;

export function initSpawnEditor(scene, gameState) {
  sceneRef = scene;
...
export function updateSpawnEditor(dt) {
...
function onLevelMouseMove(e) {
  ...
  const marker = levelMarkers.find(m =>
    ...
    for (let gi = 0; gi < wave.groups.length; gi++) {
      ...
      for (let si = 0; si < group.spawns.length; si++) {
        ...
        const ringColor = isSelected ? 0xffffff
          : (ENEMY_TYPES[spawn.type] ? ENEMY_TYPES[spawn.type].color : 0xffffff);
...
function wireEvents() {
  // Tab switching
  panel.querySelectorAll('.se-tab').forEach(tab => {
  ...
  for (const t of timingInputs) {
    ...
    const commitTimingValue = () => {
      ...
      if (isNaN(parsed)) {
        const group = getCurrentGroup();
        inp.value = (group ? group[t.prop] : slider.value) + ' ms';
        return;
      }
...
function rebuildEnemyTuningSliders() {
  ...
  sliderEls.forEach(el => {
    ...
    el.addEventListener('input', () => {
      const newVal = parseFloat(el.value);
      setNestedValue(ENEMY_TYPES[editorState.enemyType], s.key, newVal);
      const unitStr = s.unit || s.suffix || '';
      const unitDisplay = unitStr ? ' ' + unitStr : '';
  ...
  valInputs.forEach(inp => {
    ...
    const commitValue = () => {
      ...
      if (isNaN(parsed)) {
        ...
        const cur = getNestedValue(ENEMY_TYPES[editorState.enemyType], s.key);
        const unitStr = s.unit || s.suffix || '';
        const unitDisplay = unitStr ? ' ' + unitStr : '';
      ...
      setNestedValue(ENEMY_TYPES[editorState.enemyType], s.key, clamped);
      // Sync range slider
