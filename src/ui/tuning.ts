...
const SECTIONS: SliderSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Enemy Vel Mult',  config: () => LAUNCH,  key: 'enemyVelMult',     min: 1,   max: 3,   step: 0.1,
        unit: '×', tip: 'Enemy launch velocity = jump velocity × this. Higher = enemy goes higher above player.' },
      { label: 'Player Vel Mult', config: () => LAUNCH,  key: 'playerVelMult',   min: 0.5, max: 2,   step: 0.1,
        unit: '×', tip: 'Player hop velocity = jump velocity × this. Lower than enemy = player catches the ball.' },
  ...
  },
  {
    ...
    collapsed: true,
    items: [
      { label: 'Float Duration',   config: () => DUNK,  key: 'floatDuration',      min: 100, max: 800, step: 25,
        unit: 'ms', tip: 'Zero-gravity hang time — aim window before dunk.' },
      { label: 'Converge Dist',   config: () => DUNK,  key: 'floatConvergeDist',  min: 0.5, max: 4,   step: 0.25,
        unit: 'u', tip: 'Y distance threshold to trigger float phase.' },
      { label: 'Float Enemy Y',   config: () => DUNK,  key: 'floatEnemyOffsetY',  min: 0.2, max: 2,   step: 0.1,
        unit: 'u', tip: 'Enemy hovers this far above player during float.' },
      { label: 'Float Drift',     config: () => DUNK,  key: 'floatDriftSpeed',    min: 1,   max: 10,  step: 0.5,
        unit: 'u/s', tip: 'Speed enemy drifts toward player during float.' },
