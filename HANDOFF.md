...
- Room 2 ("The Foundation") — bipedal rig, melee + dash + force push, goblins, 3 pits, pit highlights on entry, no wall slam/collision damage
- Room 3 ("Physics Playground") — bipedal rig, 3 pits, 4 obstacles, wall slams + enemy collision damage + spatial force push
...
- Room highlight system: glowing corner pillars + gradient wall planes pulse on room entry (pits in Foundation, reusable for obstacles/platforms in future rooms)
...
- `src/engine/roomHighlights.ts` — reusable room highlight system (pillars + gradient wall planes)
