...
var _c1 = { r: 0, g: 0, b: 0 };
var _c2 = { r: 0, g: 0, b: 0 };
function lerpColor(target, hex1, hex2, t) {
  _c1.r = hex1 >> 16 & 255;
  _c1.g = hex1 >> 8 & 255;
  _c1.b = hex1 & 255;
  _c2.r = hex2 >> 16 & 255;
...
var AGGRO_IND = {
  ...
  heightAbove: 2.2,
  // y position above enemy
  color: 16729156,
  // red
  emissive: 16720418,
  popScale: 1.4
  // overshoot scale during pop
};
var aggroIndicators = [];
var _exclGeoBody = null;
var _exclGeoDot = null;
function getExclGeo() {
  if (!_exclGeoBody) {
...
function updateAggroIndicators(dt) {
  for (let i = aggroIndicators.length - 1; i >= 0; i--) {
    ...
    const bobY = AGGRO_IND.heightAbove + Math.sin(ind.elapsed * 1e-3 * AGGRO_IND.bobSpeed) * AGGRO_IND.bobAmp;
...
function clearAggroIndicators() {
  for (const ind of aggroIndicators) {
    ...
    if (sceneRef4) sceneRef4.remove(ind.group);
  }
...
function spawnEnemy(typeName, position, gameState2) {
  ...
  const enemy = {
    ...
    aggroed: false,
    detectionTimer: 0,
    // ms player has been continuously in cone + LOS
    // Patrol state (enemies with config.patrol)
    patrolOriginX: position.x,
    patrolOriginZ: position.z,
    patrolProgress: 0,
    // -1 to +1 along patrol axis
    patrolDir: 1,
    // +1 or -1 (forward/backward)
    patrolPauseTimer: 0,
    // ms remaining in pause at turn-around
    patrolAxis: 0,
    // rotation.y of patrol path direction (set from mesh.rotation.y at spawn)
    patrolTargetAngle: null
    // target angle for smooth turn during patrol
  };
  ...
  for (let i = gameState2.enemies.length - 1; i >= 0; i--) {
    ...
    if (!enemy.aggroed) {
      ...
      if (aggroRadius != null && aggroRadius > 0) {
        ...
        if (isInsideVisionCone(enemy.pos.x, enemy.pos.z, enemy.mesh.rotation.y, playerPos2.x, playerPos2.z, aggroRadius)) {
          const adx = playerPos2.x - enemy.pos.x;
          const adz = playerPos2.z - enemy.pos.z;
          const adist = Math.sqrt(adx * adx + adz * adz);
          if (adist > 0.1) {
            const ndx = adx / adist;
            const ndz = adz / adist;
            const terrainDist = raycastTerrainDist(enemy.pos.x, enemy.pos.z, ndx, ndz, adist);
            inConeAndLOS = terrainDist >= adist;
          } else {
            inConeAndLOS = true;
          }
        }
        if (inConeAndLOS) {
          enemy.detectionTimer = (enemy.detectionTimer || 0) + dt * 1e3;
          const threshold = VISION_CONE_CONFIG.detectionThreshold;
...
function behaviorKite(enemy, playerPos2, dt, gameState2) {
  ...
  if (enemy.sniperPhase === "idle") {
    if (dist < enemy.config.attackRange && now - enemy.lastAttackTime > enemy.config.attackRate) {
      ...
      enemy.sniperAimAngle = rotationToFace(playerPos2.x - enemy.pos.x, playerPos2.z - enemy.pos.z);
...
function getGhostGeometries() {
  if (!_ghostTorsoGeo) {
    ...
    _ghostHeadGeo2 = new THREE.SphereGeometry(P.headRadius * P.scale, 6, 4);
  }
  return { torso: _ghostTorsoGeo, head: _ghostHeadGeo2 };
}
...
function updatePlayer(inputState2, dt, gameState2) {
  ...
  if (inputState2.dash && gameState2.abilities.dash.cooldownRemaining <= 0 && endLagTimer <= 0) {
...
var ROOMS = [
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    obstacles: [
      ...
      { x: -7, z: -2, w: 1.2, h: 2, d: 1.2 },
      // left side cover
      { x: 2, z: -8, w: 1, h: 2, d: 1 },
      // mid cover (LOS break near pit)
      { x: -3, z: -16, w: 1.2, h: 2, d: 1.2 },
      // far left cover
      { x: 6, z: -14, w: 1, h: 2, d: 1 }
      // far right cover
    ],
    pits: [
      { x: 5, z: -8, w: 3, d: 3 }
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    obstacles: [
      ...
      { x: 3, z: 6, w: 1, h: 2, d: 1 },
      // entrance right cover
      { x: -8, z: -6, w: 1.2, h: 2, d: 1.2 },
      // left flank cover
      { x: 8, z: -12, w: 1, h: 2, d: 1 }
      // far right cover
    ],
    pits: [
      { x: -8, z: -8, w: 3, d: 4 },
...
var SPAWN_CONFIG = {
  ...
  minPlayerDist: 6
  // minimum spawn distance from player (prevents spawning on top of you)
};
...
function spawnEnemyArcDecal(x, z, rotationY, hitArc, hitRange, durationMs) {
  let decal = null;
...
function removeTelegraph2(telegraph) {
  if (telegraph.group.parent) {
    sceneRef7.remove(telegraph.group);
  }
...
function removeDoor() {
  hidePrompt();
  if (doorGroup && sceneRef8) {
    sceneRef8.remove(doorGroup);
...
// src/engine/dynamicHazards.ts
var HAZARD_CONFIG = {
  spawnChance: 0.5,
  // probability of spawning a pit per pack
  pitWidth: 2.5,
  // pit width
  pitDepth: 2.5,
  // pit depth (Z)
  telegraphDuration: 800,
  // ms — warning before pit materializes
  offsetRange: 3
  // max offset from pack center
};
var sceneRef9 = null;
var activeTelegraphs2 = [];
var dynamicPitVisuals = [];
function initDynamicHazards(scene2) {
  sceneRef9 = scene2;
}
function trySpawnHazard(centerX, centerZ) {
...
function updateDynamicHazards(dt) {
  for (let i = activeTelegraphs2.length - 1; i >= 0; i--) {
    ...
    if (tg.meshGroup) {
      ...
      tg.meshGroup.children.forEach((child) => {
        if (child.material) {
    ...
    if (tg.timer <= 0) {
      if (tg.meshGroup && sceneRef9) {
        sceneRef9.remove(tg.meshGroup);
        tg.meshGroup.traverse((child) => {
          if (child.geometry) child.geometry.dispose();
          if (child.material) child.material.dispose();
        });
      }
...
function clearDynamicHazards() {
  for (const tg of activeTelegraphs2) {
    if (tg.meshGroup && sceneRef9) {
      sceneRef9.remove(tg.meshGroup);
      tg.meshGroup.traverse((child) => {
        if (child.geometry) child.geometry.dispose();
  ...
  activeTelegraphs2.length = 0;
  for (const mesh of dynamicPitVisuals) {
    if (sceneRef9) {
      sceneRef9.remove(mesh);
    }
...
function isInsideExistingObstacle(x, z, w, d) {
  const bounds = getBounds();
...
function isInsideExistingPit(x, z, w, d) {
  const pits = getPits();
  const hw = w / 2, hd = d / 2;
  for (const p of pits) {
...
function createTelegraphVisual(x, z, w, d) {
  const group = new THREE.Group();
  ...
  const ringGeo3 = new THREE.PlaneGeometry(w, d);
  ringGeo3.rotateX(-Math.PI / 2);
  const ringMesh = new THREE.Mesh(ringGeo3, outlineMat);
  group.add(ringMesh);
  const fillMat = new THREE.MeshBasicMaterial({
    color: 8912930,
    transparent: true,
  ...
  const fillGeo2 = new THREE.PlaneGeometry(w * 0.9, d * 0.9);
  fillGeo2.rotateX(-Math.PI / 2);
  const fillMesh = new THREE.Mesh(fillGeo2, fillMat);
  group.add(fillMesh);
  sceneRef9.add(group);
  return group;
}
function createPitVisual(x, z, w, d) {
  const edgeThickness = 0.08;
  const edgeHeight = 0.02;
  const edgeMat = new THREE.MeshBasicMaterial({
    color: 16720452,
...
function updateRoomManager(dt, gameState2) {
  ...
  for (let i = activeTelegraphs3.length - 1; i >= 0; i--) {
    const tg = activeTelegraphs3[i];
    tg.timer -= dt * 1e3;
  ...
  if (!finalWaveAnnounced && packIndex >= budget.packs.length && activeTelegraphs3.length === 0 && aliveCount > 0) {
  ...
  if (packIndex < budget.packs.length && spawnCooldownTimer <= 0) {
    ...
    const telegraphingCount = activeTelegraphs3.reduce(
  ...
  if (!roomCleared && packIndex >= budget.packs.length && activeTelegraphs3.length === 0 && aliveCount === 0) {
    roomCleared = true;
    ...
    items: [
      {
        ...
        step: 1,
        unit: "u",
        tip: "Detection radius before goblin aggros."
      },
      {
        label: "Archer",
        config: () => ENEMY_TYPES.skeletonArcher,
        key: "aggroRadius",
        min: 2,
        max: 30,
        step: 1,
        unit: "u",
        tip: "Detection radius before skeleton archer aggros."
      },
      {
        label: "Imp",
        config: () => ENEMY_TYPES.iceMortarImp,
        key: "aggroRadius",
        min: 2,
        max: 30,
        step: 1,
        unit: "u",
        tip: "Detection radius before ice mortar imp aggros."
      },
      {
        label: "Golem",
        config: () => ENEMY_TYPES.stoneGolem,
        key: "aggroRadius",
        min: 2,
        max: 30,
        step: 1,
        unit: "u",
        tip: "Detection radius before stone golem aggros."
      },
      {
        label: "Cone Angle",
        config: () => VISION_CONE_CONFIG,
        key: "angle",
        min: 0.3,
        max: 3.14,
        step: 0.05,
        unit: "rad",
        tip: "Vision cone angle (\u03C0/3 = 60\xB0). Narrower = harder to detect."
      },
      {
        label: "Cone Opacity",
        config: () => VISION_CONE_CONFIG,
      ...
      },
      {
        ...
        step: 0.05,
        unit: "r/s",
        tip: "How fast idle enemies sweep their vision cone."
      },
      {
        label: "Scan Arc",
        config: () => VISION_CONE_CONFIG,
        ...
        step: 0.05,
        unit: "rad",
        tip: "How far idle enemies sweep from their base facing."
      },
      {
        label: "Detect Time",
        config: () => VISION_CONE_CONFIG,
        ...
        step: 25,
        unit: "ms",
        tip: "How long player must be in cone+LOS before aggro fires. 0 = instant."
      },
      {
        label: "Aggro Hold",
        config: () => VISION_CONE_CONFIG,
        key: "aggroHoldDuration",
        min: 0,
        max: 1e3,
        step: 25,
        unit: "ms",
        tip: "Red flash hold time before fade starts on aggro."
      },
      {
        label: "Patrol Dist",
        config: () => ENEMY_TYPES.goblin.patrol,
        key: "distance",
        min: 2,
        max: 15,
        step: 1,
        unit: "u",
        tip: "Total patrol distance for goblins."
      },
      {
        label: "Patrol Speed",
        config: () => ENEMY_TYPES.goblin.patrol,
        ...
        step: 0.1,
        unit: "u/s",
        tip: "Goblin patrol movement speed."
      },
      {
        label: "Pause Min",
        config: () => ENEMY_TYPES.goblin.patrol,
        ...
        step: 100,
        unit: "ms",
        tip: "Minimum pause at patrol turn-around."
      },
      {
        label: "Turn Speed",
        config: () => VISION_CONE_CONFIG,
        ...
        step: 0.25,
        unit: "r/s",
        tip: "How fast enemies rotate toward their target facing. Lower = more readable cone sweeps."
      }
    ]
  },
  // ── Dash / Abilities ──
    ...
    items: [
      {
        ...
        step: 100,
        unit: "ms",
        tip: "Dash cooldown in milliseconds. 0 = no cooldown (use end lag for lockout)."
      },
      {
        label: "Duration",
        config: () => ABILITIES.dash,
    ...
    items: [
      {
        ...
        step: 0.1,
        unit: "",
        tip: "Probability of spawning a pit per enemy pack."
      },
      {
        label: "Pit Width",
        config: () => HAZARD_CONFIG,
      ...
      },
      {
        ...
        step: 0.5,
        unit: "u",
        tip: "Depth (Z size) of dynamic pits."
      },
      {
        label: "Telegraph Dur",
        config: () => HAZARD_CONFIG,
        ...
        step: 100,
        unit: "ms",
        tip: "Warning time before pit materializes."
      },
      {
        label: "Offset Range",
        config: () => HAZARD_CONFIG,
        ...
        step: 0.5,
        unit: "u",
        tip: "Max offset from pack center for pit placement."
      }
    ]
  },
  // ── Physics ──
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        step: 1,
        unit: "u",
        tip: "Minimum distance from player when spawning enemies."
      }
    ]
  },
  {
