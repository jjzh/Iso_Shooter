...
import { fireMortarProjectile } from './mortarProjectile.js';
...
function behaviorRush(enemy, playerPos, dt) {
  ...
  if (dist > stopDist) {
    ...
    const speed = enemy.config.speed * (enemy.slowTimer > 0 ? enemy.slowMult : 1) * slideBoost;
...
function behaviorTank(enemy, playerPos, dt) {
  ...
  if (enemy.isCharging) {
    ...
    enemy.pos.x += enemy.chargeDir.x * enemy.config.speed * speedMult * slowFactor * slideBoost * dt;
    enemy.pos.z += enemy.chargeDir.z * enemy.config.speed * speedMult * slowFactor * slideBoost * dt;
    ...
    if (dist > 1) {
      _toPlayer.normalize();
      enemy.pos.x += _toPlayer.x * enemy.config.speed * slowFactor * slideBoost * dt;
      enemy.pos.z += _toPlayer.z * enemy.config.speed * slowFactor * slideBoost * dt;
    }
